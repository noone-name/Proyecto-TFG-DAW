    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-5 overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('lawyer.checklist_groups.update', $checklistGroup) }}">
                    @csrf
                    @method('PUT')
                    <label for="helper-text" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                        {{ __('Edit CheckList Group') }}</label>
            ...
            <form method="POST" action="{{ route('lawyer.checklist_groups.destroy', $checklistGroup) }}">
             ...
             <button type="submit" onclick="return confirm('{{__('¿Estas seguro de eliminar este grupo?')}}')"
                    class="text-white mt-10 bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">{{__('Delete CheckList Group')}}</button>
            </form>
        </div>
